# Roadmap: Submitting the data request and the protocol
...
tasks <- read_csv(here::here("resources/tasks.csv"),
    ...
    filter(deliverable %in% c("D1"))
```

```{r tab-milestones, eval=FALSE}
# empty_box <- as.character(fa("square", fill = "black"))
# checked_box <- as.character(fa("check-square", fill = "green"))
tasks %>% 
    filter(is_milestone) %>% 
    select(deliverable, status, task_name) %>% 
    # mutate(status = if_else(status == "" | is.na(status), empty_box, checked_box)) %>% 
