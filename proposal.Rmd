...
knitr::include_graphics("resources/fig-overview-bio.png", dpi = "retina")
```
...
health. I will be collaborating with BERTHA researchers and the center to assist
me with O1 and O2.
...
Therefore, O3 is to develop, extend, and refine NetCoupler into a usable
software package, completed under an open and reproducible scientific framework.
This O3 can be found online (https://github.com/ClemensWittenbecher/NetCoupler),
where development will publicly occur. This project builds on the idea that
developing methodological tools (O3) is most likely to succeed in direct
conjunction with its application to a specific research question (O2).

To make the algorithm usable for other research purposes, we plan on
generalising the algorithm and bundling it as an R (a statistical programming
language) software package. Once generalised, we will validate it against
simulated data and then convert it into reusable software. As O2 will contain a
large sample size and very high dimensional data, I will monitor and optimise
the computational performance of NetCoupler. I intend on having multiple
research visits to DIfE in Potsdam during the project, totalling 3-5 months, to
work intensively and closely with Dr Wittenbecher on NetCoupler. In-person
visits tend to be much more efficient and productive, especially when
encountering and solving problems. The visits will also greatly expand my
research network and introduce me to a top research institute and top European
