...
lived in Denmark from 1 January 1970 to 31 December 2020 (\~8.5 million
...
sds_variables <- read_csv(here("data/sds-registers-with-variables-to-use.csv")) %>% 
    ...
    mutate(
        ...
        time_period = case_when(
            str_detect(periode, "jan 2019 - .*") ~ periode,
            TRUE ~ str_c(start_year, " to 2020")
        )
    ) %>% 
...
sds_register_names <- c("Cancerregisteret", "IVF", "LPR_PSYK", "DRG", "PLH", "RUKS")
...
-   IVF treatments and cancer diagnoses on all persons who have lived
    and lived in Denmark in the period 1970-2020, including persons who
    ...
    -   Over the last decades, increasing trends in the incidence of
        ...
        1970-2020 is necessary, as this population will serve as the
...
<!--     -   De relevante NPU/DNK koder er listet i nedenstående tabel. -->
